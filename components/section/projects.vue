<template>
    <div class="bg-gradient-to-b from-zinc-900 to-black pb-36 -mt-1">
        <section class="container mx-auto text-white">
            <h2 class="text-2xl xl:text-5xl text-center mb-10 px-10">Recent Projects I have been working on</h2>
            <div class="flex flex-wrap space-y-10">
                <div class="w-full px-10">
                    <div
                        class="flex flex-wrap md:p-10 rounded-lg border-white items-center justify-center hover:shadow-sm hover:shadow-white duration-1000">
                        <div class="px-0 md:p-10 my-auto md:w-2/12 text-center">
                            <font-awesome-icon icon="fa-solid fa-globe" class="text-6xl" />
                        </div>
                        <div class="px-0 md:p-10 my-auto md:w-10/12 mt-5 md:mt-0">
                            <h3 class="text-xl font-bold mb-5">Flower Shop</h3>
                            <p>I am developing a project based on a flowershop needs. The goal of this project is to improve my programming skills and show to others people what I can do.</p>
                            <p class="mt-5"><strong>Used tools:</strong></p>
                            <ul class="flex flex-wrap space-x-2 mb-5">
                                <li>Vue</li>
                                <li>Vue Router</li>
                                <li>Vuetify</li>
                                <li>Vue Apollo</li>
                                <li>GraphQL</li>
                                <li>Laravel - Lighthouse</li>
                                <li>Tailwind</li>
                                <li>Github Actions</li>
                            </ul>
                            <a href="https://vue-flowershop-admin.netlify.app/" target="_blank"
                                class="border my-4 py-1 px-4 border-white rounded-md inline-block hover:bg-white hover:text-black">Visit
                                website</a>
                                <a href="https://github.com/NeftaliAguilarDev/Vue-Flowershop-Admin-Panel" target="_blank"
                                class="border ml-4 my-4 py-1 px-4 md:ml-5 border-white rounded-md inline-block hover:bg-white hover:text-black">Visit
                                repository</a>
                        </div>
                    </div>
                </div>
                <div class="w-full px-10">
                    <div
                        class="flex flex-wrap md:p-10 rounded-lg border-white items-center justify-center hover:shadow-sm hover:shadow-white duration-1000">
                        <div class="px-0 md:p-10 my-auto md:w-2/12 text-center">
                            <font-awesome-icon icon="fa-solid fa-globe" class="text-6xl" />
                        </div>
                        <div class="px-0 md:p-10 my-auto md:w-10/12 mt-5 md:mt-0">
                            <h3 class="text-xl font-bold mb-5">Visit Puerto Vallarta</h3>
                            <p>I was in charged for this project, it is a website for the destiny where I live (Puerto
                                Vallarta), It was developed by me and another person focused on backend</p>
                            <p class="mt-5"><strong>Used tools:</strong></p>
                            <ul class="flex flex-wrap space-x-2 mb-5">
                                <li>Laravel</li>
                                <li>Bootstrap</li>
                                <li>Jquery</li>
                                <li>Vue</li>
                                <li>Lazy load</li>
                                <li>Stylus</li>
                            </ul>
                            <a href="https://visitpuertovallarta.com" target="_blank"
                                class="border py-1 px-4 border-white rounded-md inline-block hover:bg-white hover:text-black">Visit
                                website</a>
                        </div>
                    </div>
                </div>
                <div class="w-full px-10">
                    <div
                        class="flex flex-wrap md:p-10 rounded-lg border-white items-center justify-center hover:shadow-sm hover:shadow-white duration-1000">
                        <div class="px-0 md:p-10 my-auto md:w-2/12 text-center">
                            <font-awesome-icon icon="fa-solid fa-globe" class="text-6xl" />
                        </div>
                        <div class="px-0 md:p-10 my-auto md:w-10/12 mt-5 md:mt-0">
                            <h3 class="text-xl font-bold mb-5">Quiero Flores - Website</h3>
                            <p>I developed this ecommerce website for a flower shop located in Puerto Vallarta, Mexico.</p>
                            <p class="mt-5"><strong>Used tools:</strong></p>
                            <ul class="flex flex-wrap space-x-2 mb-5">
                                <li>Bootstrap</li>
                                <li>Gulp</li>
                                <li>Jquery</li>
                                <li>Git</li>
                                <li>Lazy load</li>
                                <li>Stylus</li>
                            </ul>
                            <a href="https://quieroflores.com.mx" target="_blank"
                                class="border py-1 px-4 border-white rounded-md inline-block hover:bg-white hover:text-black">Visit
                                website</a>
                        </div>
                    </div>
                </div>
                <div class="w-full px-10">
                    <div
                        class="flex flex-wrap md:p-10 rounded-lg border-white items-center justify-center hover:shadow-sm hover:shadow-white duration-1000">
                        <div class="px-0 md:p-10 my-auto md:w-2/12 text-center">
                            <font-awesome-icon icon="fa-solid fa-globe" class="text-6xl" />
                        </div>
                        <div class="px-0 md:p-10 my-auto md:w-10/12 mt-5 md:mt-0">
                            <h3 class="text-xl font-bold mb-5">Quiero Flores - Web System</h3>
                            <p>I developed a custom web admin panel for managing all the business logic.</p>
                            <p>This contains diffrente modules like: Orders, Customers, Drivers, Florists, Expenses, Reports
                                and others...</p>
                            <p>I also deployed this project, I made the configuration a droplet in Digital Ocean using
                                Ubuntu 18 and Nginx as a server.</p>
                            <p class="mt-5"><strong>Used tools:</strong></p>
                            <ul class="flex flex-wrap space-x-2 space-x-reverse mb-5">
                                <li>Laravel</li>
                                <li>Jquery</li>
                                <li>Datatables</li>
                                <li>Woocomerce Webhooks</li>
                                <li>Bootstrap</li>
                            </ul>
                            <span><strong>Only company personal can navigate into the project</strong></span>
                        </div>
                    </div>

                </div>
            </div>
        </section>
    </div>
</template>

<script setup>
</script>

<style scoped></style>